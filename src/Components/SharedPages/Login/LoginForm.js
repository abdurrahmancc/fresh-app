...
const LoginForm = ({ handleLoginMOdal, setIsOpenModal }) => {
  ...
  const [isValidToken, tokenLoading] = useValidToken(user || eUser);
  ...
  useEffect(() => {
    if (gUser) {
      (async () => {
        try {
          const photoURL = gUser?.user?.photoURL;
          const username = gUser?.user?.email;
          ...
          <div className="card-body gap-0 p-0">
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <div className="form-control mt-5">
                ...
                >
                  {isLogin || loading || eLoading || tokenLoading ? (
